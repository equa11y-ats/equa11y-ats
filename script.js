// Mobile menu functionality
        ...
        function showSlide(n) {
            ...
            // Focus on the active indicator for keyboard users
            indicators[currentSlide].focus();
        }

        // Button Controls
        document.querySelector('.prev').addEventListener('click', () => {
            showSlide(currentSlide - 1);
            // Set focus to the CTA button in the active slide
            setTimeout(() => {
                const activeSlide = document.querySelector('.slide.active');
                if (activeSlide) {
                    const ctaButton = activeSlide.querySelector('.cta-button');
                    if (ctaButton) {
                        ctaButton.focus();
                    }
                }
